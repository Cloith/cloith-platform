...
def main():
    ...
    with console.status("[bold green]Initializing secure environment...[/bold green]"):
        ...
        subprocess.run(["sudo", "rm", "-f", "/var/run/docker.pid"], capture_output= True)
        subprocess.run(["sudo", "rm", "-f", "/var/run/docker.sock"], capture_output= True)
        time.sleep(1)
    ...
    try:
        main()
            ...
            with console.status("[bold yellow]Logging out of bitwarden...[/bold yellow]"):
                subprocess.run(["bw", "logout"], capture_output=True)
            with console.status("[bold yellow] Cleaning docker session...[bold yellow]"):
